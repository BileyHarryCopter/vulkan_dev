...
namespace VKPipeline
{
    ...
    Pipeline::Pipeline(VKDevice::Device& device, const PipelineConfigInfo& configInfo, bool useMeshShaders)
                        : device_{device.get_logic()}, useMeshShaders_{useMeshShaders}
    {
        if (useMeshShaders_)
            createMeshShaderPipeline(configInfo);
        else
    ...
    Pipeline::~Pipeline()
    {
        if (useMeshShaders_)
        {
            if (taskshadermodule_ != VK_NULL_HANDLE)
                vkDestroyShaderModule(device_, taskshadermodule_, nullptr);
            if (meshshadermodule_ != VK_NULL_HANDLE)
                vkDestroyShaderModule(device_, meshshadermodule_, nullptr);
        }
        else
        {
    ...
    void Pipeline::createMeshShaderPipeline(const PipelineConfigInfo& configInfo)
    {
        assert(configInfo.pipelineLayout != VK_NULL_HANDLE &&
                "Cannot create mesh shader pipeline: no pipelineLayout provided in configInfo");
        assert(configInfo.renderPass != VK_NULL_HANDLE &&
                "Cannot create mesh shader pipeline: no renderPass provided in configInfo");

        auto taskShaderCode = Service::readfile(TASK_SHADER_FILE_NAME);
        auto meshShaderCode = Service::readfile(MESH_SHADER_FILE_NAME);
        auto fragShaderCode = Service::readfile(FRAG_SHADER_FILE_NAME);

        taskshadermodule_ = createShaderModule(taskShaderCode, device_);
        meshshadermodule_ = createShaderModule(meshShaderCode, device_);
        fragshadermodule_ = createShaderModule(fragShaderCode, device_);
        ...
        taskShaderStageInfo.module = taskshadermodule_;
        ...
        meshShaderStageInfo.module = meshshadermodule_;
        ...
        // No vertex input state needed for mesh shaders
        VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
        vertexInputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        ...
        pipelineInfo.pStages = shaderStages;
        pipelineInfo.pVertexInputState = &vertexInputInfo;
        pipelineInfo.pInputAssemblyState = nullptr;  // Not used with mesh shaders
        pipelineInfo.pViewportState = &configInfo.viewportInfo;
        pipelineInfo.pRasterizationState = &configInfo.rasterizationInfo;
        pipelineInfo.pMultisampleState = &configInfo.multisampleInfo;
